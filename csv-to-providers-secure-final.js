 ...
 * - SECURITY: API key masking and output sanitization
...
function log(level, ...args) {
  if (levels[level] <= (levels[LOG_LEVEL] ?? 2)) {
    ...
    console.log(`[${ts}] [${level.toUpperCase()}]`, ...sanitizedArgs);
  }
}
...
function maskApiKey(key) {
  if (!key || key === null || key === undefined || key === '') return null;
...
/**
 * Sanitize log arguments to prevent injection
 */
...
function sanitizeError(error) {
  ...
  if (typeof error === 'string') {
    ...
    if (genericMessages[errorCode]) {
      return genericMessages[errorCode];
    }
    // Generic error for unknown strings
  ...
  if (error && typeof error === 'object') {
    ...
    const sanitized = {
      message: error.message ? 'Operation failed' : 'Unknown error',
 ...
 */
function sanitizeInput(str, fieldName = '') {
  ...
  } else if (fieldName.toLowerCase().includes('name') || fieldName.toLowerCase().includes('provider')) {
    // Only allow alphanumeric, spaces, and basic punctuation for names
    sanitized = sanitized.replace(/[^\w\s\-.,]/g, '');
  } else if (fieldName.toLowerCase().includes('key') || fieldName.toLowerCase().includes('token')) {
    // For API keys, only allow certain characters
...
/**
 * Sanitize output object to remove sensitive data
 */
function sanitizeOutput(obj) {
  ...
  function sanitizeObject(o) {
    for (const key in o) {
      ...
      const isSensitiveField = key.toLowerCase().includes('api_key') || 
...
async function searchHuggingFaceByModelName(modelName) {
  ...
  try {
    ...
    log('debug', `HuggingFace search failed for ${modelName}:`, sanitizeError(error));
  }
...
class HuggingFaceAPI {
  constructor(apiKey = '') {
    ...
    this.headers = {
      'User-Agent': 'Your-PaL-MoE/0.3-Secure',
  ...
  async searchModels(query = '', options = {}) {
    ...
    try {
      ...
      log('error', `HuggingFace model search failed:`, sanitizeError(error));
      return [];
    }
  }
  ...
  async getModelDetails(modelId) {
    ...
    try {
      ...
      log('warn', `Failed to get details for model ${modelId}:`, sanitizeError(error));
      return null;
    }
  }
    ...
    for (let attempt = 0; attempt <= RETRIES; attempt++) {
      try {
        ...
        if (attempt < RETRIES) {
          ...
          log('warn', `HF API request failed (attempt ${attempt + 1}/${RETRIES + 1}):`, sanitizeError(err));
...
async function httpGet(url, headers = {}, timeout = TIMEOUT_MS, retries = RETRIES) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      log('warn', `GET ${url} failed (attempt ${attempt + 1}/${retries + 1})`,
          sanitizeError(status || err.code || err.message),
  ...
  log('error', `Final failure for ${url} after ${retries + 1} attempts:`,
      sanitizeError(lastErr?.response?.status ? `HTTP ${lastErr.response.status}` : lastErr?.code || 'Unknown error'),
      sanitizeError(lastErr?.message || 'No error message available'));
...
async function resolveModelsForRow(row) {
  ...
  try {
    ...
    if (isDelimitedList) {
      ...
      for (const name of modelNames) {
        try {
          ...
          log('warn', `Failed to process model ${name} from delimited list:`, sanitizeError(error));
          // Add fallback model entry
          models.push({
            id: normalizeModelId(name),
            capabilities: inferCapabilitiesFromModelName(name),
            meta: { type: 'delimited-list', original: name, error: sanitizeError(error) }
          });
        }
      }
      ...
      if (HF_API_KEY) {
        ...
        for (const model of models) {
          try {
            ...
            log('debug', `Could not enhance model ${model.id}:`, sanitizeError(error));
          }
        }
      }
    ...
    log('error', `Failed to process models from ${modelField}:`, sanitizeError(e));
    return [];
  }
}
...
async function generateProvidersJSON(csvPath, outputPath) {
  ...
  await Promise.all(validRows.map((row, idx) => limit(async () => {
    try {
      ...
      // Enhanced URL validation for base URL
      ...
      log('error', `Row ${idx + 1}: Unexpected error processing provider:`, sanitizeError(error));
      errorCount++;
    }
  })));
  ...
  const json = JSON.stringify(sanitizedResults, null, 2);
  ...
  const trigger = () => {
    ...
    timer = setTimeout(async () => {
      try {
        ...
        log('error', 'Error during hot reload:', sanitizeError(e));
      }
    }, 350); // debounce
  };
...
async function main() {
  try {
    ...
    // Set up environment with test API keys for demonstration
    ...
    log('error', 'Failed to generate providers.json:', sanitizeError(e));
    process.exitCode = 1;
  }
  ...
  if (WATCH) {
    watchCSV(CSV_PATH, async () => {
      try {
        ...
        log('error', 'Hot reload generation failed:', sanitizeError(e));
      }
    });
  }
}
